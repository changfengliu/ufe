...
module.exports = {
	 ...
	 * 更新已存在的 api 设置
	 */
	updateApi(db, oldApi, settings){
		db.get('apis').find({name: oldApi}).assign(settings).write()
		return db
	},
	/**
	 * 添加与更新共用
	 */
	saveApi(db, oldApi, settings){
		if(oldApi){
			this.updateApi(db, oldApi, settings)
		} else {
			this.addApi(db, settings)
		}
	},
	/**
	 * 删除 api
	 */
	removeApi(db, api){
		db.get('apis').remove({name: api}).write()
		return db
	}
}
